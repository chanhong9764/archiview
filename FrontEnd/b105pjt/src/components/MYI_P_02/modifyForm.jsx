    ...
    const handleRecordStop = () => {
        ...
        stopRecording(
            ...
            (resp) => {
                ...
                // 세션 및, 퍼블리셔 종료 로직
                if (publisher) {
                    publisher.stream.getVideoElement().parentNode.removeChild(publisher.stream.getVideoElement());
...
const BtnGroupInsert = ({ id, script, url }) => {
    ...
    return (
        <div className="Insert-btn-group">
            {role !== 'ROLE_ADMIN' && (
                <Button variant="outlined" startIcon={<ModeEditIcon />} color="success" onClick={handleEdit}>
                    수정
                </Button>
            )}
            <Button variant="contained" endIcon={<DeleteForeverIcon />} color="error" onClick={handleDelete}>
                삭제
            </Button>
        </div>
    );
