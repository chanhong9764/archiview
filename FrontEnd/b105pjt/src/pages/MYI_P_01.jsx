    ...
    <Container sx={{ mt: 4, mb: 4 }}>
      ...
      {questions.map((question, index) => (
        <Paper key={index} sx={{ my: 2, p: 2, boxShadow: theme.shadows[3] }}>
          <Typography variant="h6" sx={{ mb: 2, color: theme.palette.primary.main }}>{question.content}</Typography>
          <Accordion sx={accordionStyles}>
            <AccordionSummary
              ...
              id="panel-header"
            >
              <Typography variant="subtitle1" color="textSecondary">답변 보기</Typography>
            </AccordionSummary>
            <AccordionDetails sx={{ flexDirection: 'column', padding: theme.spacing(2) }}>
              {question.replies.map((reply, replyIndex) => (
                <Box key={reply.id} sx={{ my: 2 }}>
                  ...
                  <Typography variant="subtitle1" sx={{ mt: 2, fontWeight: 'bold' }}>{reply.user_id}</Typography>
                  <Typography variant="body2" sx={{ my: 1 }}>{reply.script}</Typography>
                  <Typography variant="caption" sx={{ mb: 2, color: theme.palette.text.secondary }}>{reply.created_at}</Typography>
                </Box>
              ))}
              <Button
                variant="contained"
